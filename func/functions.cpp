...
float MINDIST(float *p, float *bounces, int dim)
{
    ...
    for(i = 0; i < dim; i++)
    {
		if (p[i] < bounces[2*i]) //i=0 , p_x < mbr_x1,mbr_x2 , i = 1 p_y < mbr_y1,mbr_y2
			r = bounces[2*i];
		else
		{
...
{	
		...
		*/
		...
		//disjoint
	if(bounces1[1] < bounces[0] && bounces1[3] < bounces[2])
		return pow(bounces[0]-bounces1[1],2)+pow(bounces[2]-bounces1[3],2);
	if(bounces1[1] < bounces[0] && bounces1[2] > bounces[3])
		return pow(bounces[0]-bounces1[1],2)+pow(bounces[3]-bounces1[2],2);
	if(bounces1[0] > bounces[1] && bounces1[3] < bounces[2])
		return pow(bounces[1]-bounces1[0],2)+pow(bounces[2]-bounces1[3],2);
	if(bounces1[0] > bounces[1] && bounces1[2] > bounces[3])
		return pow(bounces[1]-bounces1[0],2)+pow(bounces[3]-bounces1[2],2);
	

		//overlap
		if (((bounces1[0]>=bounces[0] && bounces1[0]<=bounces[1]) || (bounces1[1]>=bounces[0] && bounces1[1]<=bounces[1]) || (bounces1[0]<=bounces[0] && bounces1[1]>=bounces[1])) && ((bounces1[2]>=bounces[2] && bounces1[2]<=bounces[3]) || (bounces1[3]>=bounces[2] && bounces1[3]<=bounces[3]) || (bounces1[2]<=bounces[2] && bounces1[3]>=bounces[3])))
			return 0.0;

		//one axis overlap
		if (bounces1[3] <= bounces[2])
			return pow(bounces[2]-bounces1[3],2);
		if (bounces1[2] >= bounces[3])
			return pow(bounces[3]-bounces1[2],2);
		if (bounces1[1] <= bounces[0])
			return pow(bounces[0]-bounces1[1],2);
		if (bounces1[0] >= bounces[1])
			return pow(bounces[1]-bounces1[0],2);
...
{	
	...
	if(p.x1 == p.x2 && p.y1 == p.y2)
	{
		...
		if(bounces[0] > p.x1 && bounces[2] > p.y1)	return pow(bounces[0]-p.x1,2)+pow(bounces[2]-p.y1,2);
		if(bounces[0] > p.x1 && bounces[3] < p.y1)  return pow(bounces[0]-p.x1,2)+pow(bounces[3]-p.y1,2);
		if(bounces[1] < p.x1 && bounces[2] > p.y1)	return pow(bounces[1]-p.x1,2)+pow(bounces[2]-p.y1,2);
		if(bounces[1] < p.x1 && bounces[3] < p.y1)	return pow(bounces[1]-p.x1,2)+pow(bounces[3]-p.y1,2);
...
{  
		...
		d1 = pow(bounces[1]-bounces1[0],2)+pow(bounces[3]-bounces1[2],2);
		d2 = pow(bounces[1]-bounces1[0],2)+pow(bounces[2]-bounces1[3],2);
		d3 = pow(bounces[0]-bounces1[1],2)+pow(bounces[3]-bounces1[2],2);
		d4 = pow(bounces[0]-bounces1[1],2)+pow(bounces[2]-bounces1[3],2);
