...
void LinList::insert(Linkable *f)
{
    ...
    sd->prev = NULL;
	if (first != NULL){
		//add by shirley to locate the current position after insertion
		//akt = first;
		akt_index ++;
	    first->prev = sd;
	}
...
//added by Shirley
void LinList::printnew()
{
    SLink *cur = first;

    while (cur != NULL)
    {
	  printf("%d %f %f\n", cur->d->son, cur->d->bounces[0],  cur->d->bounces[3]);
	  cur = cur->next;
    }
}
...
//---added for validity region queries by Bobby-------------------------
void Linkable::clone(Linkable *old)
{
	son = old->son;
	dimension = old->dimension;
	level = old->level;
	float distanz = old->distanz;

	int i;
	for (i = 0; i < 2*dimension; i++)
		bounces[i] = old->bounces[i];
}