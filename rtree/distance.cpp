...
//the returned period is stored in [_ts, _te]
{
	...
	for (int i = 0; i < _dim; i ++)
	{
		...
		if (tlr >= 0 && trl >= 0)
		{
			tsi[i] = min(tlr, trl); tei[i] = max(tlr, trl);
		}
		else
			if (max(tlr, trl) >= 0)
			{
				...
				{
					tsi[i] = 0; tei[i] = max(tlr, trl);
				}
				else
				{
					tsi[i] = max(tlr, trl); tei[i] = MAXREAL;
				}
			}
			else
			{
	...
	for (int i = 0; i < _dim; i ++)
	{
		_ts = max(_ts, tsi[i]); _te = min(_te, tei[i]);
	}
	if (_ts > _te)
	{
		_ts = _te = MAXREAL;
